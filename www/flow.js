...
(function () {
  ...
  async function init() {
    ...
    // 自动聚焦搜索框（仅 Electron 桌面端，iOS 不自动聚焦避免弹出键盘）
    if (!isCapacitor) {
      setTimeout(() => {
  ...
  async function exportData() {
    try {
      ...
      } else if (isCapacitor && typeof Capacitor !== 'undefined') {
        // Capacitor 环境：使用分享功能
        try {
          const { Filesystem, Directory } = await import('https://cdn.jsdelivr.net/npm/@capacitor/filesystem@6/+esm');
          const { Share } = await import('https://cdn.jsdelivr.net/npm/@capacitor/share@6/+esm');
          
          const fileName = `flow-data-${new Date().toISOString().split('T')[0]}.json`;
          ...
          console.error('Capacitor 导出失败:', capError);
          // 降级到浏览器方式
